...
contract UserEntry is CMCEnabled {
    ...
    function authorByAddress(address _address) external view returns  (
        bytes32 name,
        bytes32 email,
        bool registered) {
          return ipfsLogic(ContractProvider(CMC).contracts("ipfsLogic")).authorByAddress(_address);
    }
    ...
    function getBookById(uint _id) external view returns (
        bytes32 bookhash,
        bytes32 thumbHash,
        bytes32 title,
        address author,
        bytes32 IPPR) {
          return ipfsLogic(ContractProvider(CMC).contracts("ipfsLogic")).getBookById(_id);
    }
}
