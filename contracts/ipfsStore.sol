...
contract ipfsStore is CMCEnabled {
    ...
    function getBookById(uint _id) external view isCMCEnabled("ipfsLogic") returns (bytes32 bookHash, bytes32 thumbHash, bytes32 title, address author, bytes32 IPPR) {
      return (books[_id].bookHash, books[_id].thumbHash, books[_id].title, books[_id].author, books[_id].IPPR);
    }
}
